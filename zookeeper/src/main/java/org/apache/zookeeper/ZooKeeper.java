...
public class ZooKeeper implements AutoCloseable {
    ...
    public abstract class WatchRegistration {
        ...
        public void register(int rc) {
            if (shouldAddWatch(rc)) {
                //10--zk客户端ZKWatchManager的三种watch
     ...
     *             if an invalid chroot path is specified
     */
     //--ClientCntn连接时可以同时制定多台服务器，根据算法链接某一台，当某台服务器挂了，会自动链接其它服务器，hostProvider实现
    //--ZooKeeper集群的服务器地址列表（connectString）
    //--最终会引出三个时间设置：和服务端协商后的sessionTimeout、readTimeout、connectTimeout
    //服务器端使用协商后的sessionTimeout：即超过该时间后，客户端没有向服务器端发送任何请求（正常情况下客户端会每隔一段时间发送心跳请求，此时服务器端会从新计算客户端的超时时间点的），
    ...
    //--具有相同实际路径的节点已经存在于ZooKeeper中抛出异常
    //--如果父节点在ZooKeeper中不存在抛出异常
    //--临时节点不能有子节点。如果父节点是给定的path是临时的抛出异常
    //--同步，没有回调
        ...
        throws KeeperException, InterruptedException
    {
        ...
        //3:服务端响应后结果填充到createRequest实体返回给客户端
        //Clinetcnzx和ClientCnxnSocketNetty公用一个outgoingQueue，后者启动发送守护线程，将outgoingQueue中的request发送给zookeeperserver，
        //ClientCnxnSocketNetty启动接受线程线程接受服务器返回结果，
        //--返回数据触发ClientCnxnSocketNetty中启动的ZKClientHandler的Messagereceiver???（channelRead0),这个是回调
        //最终调用ClientCnxn中的sendThread的readResponse，调用finishPacket唤醒之前的线程wait(submitRequest)
    ...
    public byte[] getData(final String path, Watcher watcher, Stat stat)
        throws KeeperException, InterruptedException
     {
        ...
        // the watch contains the un-chroot path
        WatchRegistration wcb = null;
        //--对watcher定义了WatchRegistration，将path和watcher关联起来，这样就知道哪个path的数据变化时触发该watcher
        if (watcher != null) {
