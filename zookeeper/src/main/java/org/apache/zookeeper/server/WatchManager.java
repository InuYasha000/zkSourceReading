 ...
 */
//10--服务端的watcher会最终到datatree的dataWatches（WatchManager含有两个map，watchTable和watch2Paths）
class WatchManager {
    ...
    //--将watcher记录到本地列表，watcher和path关联起来，通过triggerwatch触发
    ...
    //10--1:封装watchedEvent，2：查询watcher，3：调用process触发watcher
    Set<Watcher> triggerWatch(String path, EventType type, Set<Watcher> supress) {
        ...
        for (Watcher w : watchers) {
            ...
            //--回调NettyServerCnxn的process方法，NettyServerCnxn实现Watcher接口
            //10--请求头标记-1，表示这是一个通知
            //10--watchedevent包装秤一个WatcherEvent，便于网络序列化
            //10--向客户端发送通知
            w.process(e);
        }
        return watchers;
