...
//--响应Zookeeper客户端请求类
    ...
    //--NettyServerCnxn收到客户端请求后会自动调用ZookeeperServer的processPacket来执行客户端命令
    //7--PrepRequestProcessor -> SyncRequestProcessor -> FinalRequestProcessor
    //7--服务端有两种消息处理，Request和QuorumPacket
public class ZooKeeperServer implements SessionExpirer, ServerStats.Provider {
    ...
    //7--写请求入ZookeeperServer的outstandingChanges队列,参考PrepRequestProcessor
    //7--outstandingChanges主要用于FinalRequestProcessor的processorRequest方法中，
    // 用来确保FinalRequestProcessor是按照outstandingChanges的顺序来执行Request
    //--都是写请求
    ...
    //生成sessionId（sessionId在基准之上逐个递增）
    ...
    //7--判断ZookeeperServer的处理包流程是否准备就绪，没有则等待1秒，看是否抛出异常，否则调用firstprocess
    //7--firstProcess根据zookeeperserver子类不同指向的也不同，Leader->LeaderRequestProcessor,Follower->FollowerRequestProcessor
    //PrepRequestProcessor -> SyncRequestProcessor -> FinalRequestProcessor
    ...
    //7--处理请求连接，被NettyServerCnxn或者NioServerCnxn在网络连接请求时调用，都实现了ServerCnxn接口
    //会话创建请求
    public void processConnectRequest(ServerCnxn cnxn, ByteBuffer incomingBuffer) throws IOException {
        ...
        //readOnly服务端只能接收来自readOnly型客户端的请求
        ...
        //正常情况下，服务端ZIXD大于客户端，否则不接受会话创建请求
        ...
        //协商sessionTimeout，zk服务端根据自己的超时时间限制来确定最终的超时时间
    ...
    //7--客户端请求到达调用processPacket处理该请求，已经建立了和客户端的连接
    //7--从nettyservercnxn收到request，调用submitRequest方法
    //7--submitRequest-->firstProcessor
    ...
    //7--处理客户端写操作
    //7--通过ZKdatabase类实现具体逻辑
    //7--FinalRequestProcessor调用，写操作和读操作有很大区别，写操作提供了统一的入口方法processTxn
