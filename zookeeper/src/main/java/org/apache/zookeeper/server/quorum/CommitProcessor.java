...
public class CommitProcessor extends ZooKeeperCriticalThread implements
        ...
        try {
            while (!stopped) {
                ...
                while (!stopped && !isWaitingForCommit() &&
                    ...
                    //7--是否需要commit，只有写操作才需要被commit，才会被移到commitRequest,等待被处理？？？没看到哪里写的  写操作
                    //7--queuedRequests放入
                    if (needCommit(request)) {
                        nextPending.set(request);
                    } else {
                        //--发送给下一个执行链 nextProcessor->FinalRequestProcessor
    ...
    //--将请求放入committedRequest队列，等待run线程往FinalRequestProposal进行真正的处理
    ...
    //7--有请求到达时直接放入QueueRequest请求队列
    //7--然后唤醒等待线程开始处理队列中的请求
    @Override
