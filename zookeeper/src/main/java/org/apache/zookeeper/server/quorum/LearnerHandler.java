...
public class LearnerHandler extends ZooKeeperThread {
    ...
    public void run() {
        try {
            ...
            //6--leader根据followerinfo的值计算出来的新的Epoch
            long newEpoch = leader.getEpochToPropose(this.getSid(), lastAcceptedEpoch);
            //根据新的epoch计算新的ledaer的zxid
            long newLeaderZxid = ZxidUtils.makeZxid(newEpoch, 0);

            //6--然后给follower发送LeaderInfo，将新的zxid发给Follower，等待Follower的ackEpoch包，收到ACKEPOCH包后，说明已经同步(这里不是同步事务，是同步epoch)
            ...
            //6--判断是是否要发SNAP包给follower
            //6--follower事务比leader少，不发snap包，发diff包，同时将需要补充的事务通过Proposal和COMMIT发给Follower执行
            //6--syncFollower()->queueCommittedProposals()->startSendingPackets()
            //6--queuedPackets队列中就是发送给learner的消息
            ...
            //6--等Follower回复第一个ack包，调用该方法告诉leader该follower已经同步
            ...
            // using the data
            //6--leader发送UPTODATE包告诉Follower，通知Follower数据全部更新完毕
            LOG.debug("Sending UPTODATE message to " + sid);      
            queuedPackets.add(new QuorumPacket(Leader.UPTODATE, -1, null, null));

            //6--注册和事务同步后，LearnerHandler进入循环处理QuorumPacket
            //6--leader收到写事务请求，发送Proposal消息给Follower，通知Follower准备执行该事务(后续还需要Commit命令提交)，
            //6--Follower收到客户端的写请求事务，Follower发送Request消息给Leader，让Leader主持改写事务在集群间群发，确保所有机器都被同步
            while (true) {
                qp = new QuorumPacket();
                ia.readRecord(qp, "packet");
                ...
                switch (qp.getType()) {
                case Leader.ACK://--follower收到proposal并写到磁盘后会发送ack给leader
                ...
                case Leader.REVALIDATE:
                    ...
                    break;
                case Leader.REQUEST://--follower收到客户端写命令时发送REQUEST给Leader
