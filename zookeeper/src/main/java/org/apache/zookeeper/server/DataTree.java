...
public class DataTree {
    ...
    //--path--被删除节点的名称
    //--zxid--当前zxid
    //--删除指定路径的节点，同时修改该节点父节点的子节点列表，触发watcher事件（一种是该节点自身的，一种是父节点的getChildren监听的watcher）
    ...
    //8--调用datatree方法获取节点的关联数据
    //8--可以指定watcher，变更数据时触发watcher
    //8--客户端监听的watcher会被触发，该watcher定义的process接口会被调用
    //--将watcher添加到datawatcher列表(WatchManager)
    ...
    //8--返回指定节点名称的子节点，可指定参数watcher，节点的子节点列表变更时触发watcher
    ...
    public ProcessTxnResult processTxn(TxnHeader header, Record txn, boolean isSubTxn)
    {
        ...
        try {
            ...
            switch (header.getType()) {
                case OpCode.create://8--触发该节点的父节点的getChildren方法注册的Watcher
                ...
                case OpCode.createContainer:
                    ...
                    break;
                case OpCode.delete:
                case OpCode.deleteContainer://8--删除节点，触发两种watcher，一个是该节点的父节点的getChildren方法注册的Watcher，而是该节点自身的getdata注册的watcher
                    DeleteTxn deleteTxn = (DeleteTxn) txn;
                    rc.path = deleteTxn.getPath();
                    deleteNode(deleteTxn.getPath(), header.getZxid());
                    break;
                case OpCode.reconfig:
                case OpCode.setData://8--触发注册在该节点上的watcher
                    ...
                    break;
                case OpCode.setACL://8--设置节点的acl，该方法不触发watcher
    ...
    //8--将datatree保存到磁盘，循环将数据节点序列化到磁盘，serializeNode，这是deserialize的逆方法
