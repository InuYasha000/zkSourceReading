 ...
 */
//--FollowerZookeeperServer的firstprossor指向
public class FollowerRequestProcessor extends ZooKeeperCriticalThread implements
    ...
    public void run() {
        try {
            while (!finished) {
                ...
                switch (request.type) {
                case OpCode.sync://7--写操作发送request消息
    ...
    //--简单的将请求包Request入队列queuedRequests，等待run主线程处理
