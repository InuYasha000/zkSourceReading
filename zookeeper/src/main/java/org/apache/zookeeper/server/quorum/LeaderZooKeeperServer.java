 ...
 */
//--leader启动LeaderZookeeperServer来接受客户端请求
//7--LeaderZooKeeperServer:LeaderRequestProcessor->PrepRequestProcessor->ProposalRequestProcessor->commitProcessor->toBeAppliedProcessor->FinalRequestProcessor
//7--FollowerZooKeeperServer:FollowerRequestProcessor->commitProcessor->FinalRequestProcessor->
//SyncRequestProcessor->SendAckRequestProcessor
//7--ObserverZooKeeperServer:firstProcesor->commitProcessor->finalProcessor
//7--ZooKeeperServer:PrepRequestProcessor -> SyncRequestProcessor -> FinalRequestProcessor

//7--LeaderRequestProcessor:Leade调用链开始
//7--PrepRequestProcessor：写操作预准备，创建Request内容
//7--ProposalRequestProcessor：将写操作封装成Proposal包光波导所有Follower
//7--CommitProcessor：判断写操作是否可以直接执行
//7--toBeappliedProcessor：将commit后的Request移到ToBeApplied列表，等待 发送给FinalRequestProcessor
//7--FinalRequestProcessor：真正执行Zookeeper命令的地方
//7--SyncRequestProcessor：将写操作持久化到磁盘
//7--AckRequestProcessor：当proposal持久化到磁盘，向本机发送ack包

//7--SyncRequestProcessor和AckRequestProcessor是在ProposalRequestProcessor内部被创建的，
// 当Leader要处理proposal命令时先自己调用SyncRequestProcessor持久化，然后通过AckRequestProcessor直接告诉Leader处理ack逻辑（不经过QuorumPacket传递）
public class LeaderZooKeeperServer extends QuorumZooKeeperServer {
    ...
    //7--请求连设置顺序
    //7--LeaderRequestProcessor->PrepRequestProcessor->ProposalRequestProcessor->commitProcessor->toBeAppliedProcessor->FinalRequestProcessor
    //7--LeaderRequestProcessor->PrepRequestProcessor->ProposalRequestProcessor->SyncRequestProcessor->AckRequestProcessor
    @Override
