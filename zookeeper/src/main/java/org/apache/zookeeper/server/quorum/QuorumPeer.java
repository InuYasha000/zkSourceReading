...
public class QuorumPeer extends ZooKeeperThread implements QuorumStats.Provider {
    ...
    public synchronized void start() {
        ...
        //恢复本地数据，也就是加载磁盘快照和事务日志
        loadDataBase();
        //启动ServerCnxnFactory：网络连接工厂
    ...
    synchronized public void startLeaderElection() {
      ...
      //      throw new RuntimeException("My id " + myid + " not in the peer list");
        //}
        //选举算法，0,1,2都已经被废弃掉了，会走3
        //0:基于UDP网络连接,不可靠的网络连接，已经废弃掉
        if (electionType == 0) {
            try {
    ...
    protected Election createElectionAlgorithm(int electionAlgorithm){
        ...
        case 3:
            ...
            if(listener != null){
                listener.start();
                //--fastLeadElection
                //--初始化FastLeaderElection中的sendqueue和recvqueue两个队列
                //--初始化FastLeaderElection中的QuorumCnxManager
                //--初始化Messenger中WorkerSender线程和WorkerReceiver线程
                FastLeaderElection fle = new FastLeaderElection(this, qcm);
                //--启动把消息放进QueueSendMap中的线程-不是发送消息的线程
                //--启动接受线程（注意这个和上面消息放入queueSendMap的线程不一样）
                //--启动FastLeaderElection中的QuorumCnxManager线程
                fle.start();
                le = fle;
            } else {
    ...
    public void run() {
        ...
        try {
            ...
            while (running) {
                switch (getPeerState()) {
                    //刚启动就是LOOKING状态，也就是没有leader，此时会去选举leader
                    //当启动后发现有leader，自己就是follower
                case LOOKING:
                    LOG.info("LOOKING");
                    ...
                    if (Boolean.getBoolean("readonlymode.enabled")) {
                        ...
                        try {
                            ...
                            //--选举逻辑入口
                            //在这里知道谁是leader后会再次循环，然后就开始做自己的事情
